...
class substanceGen(pg.sprite.Sprite):  #class for chemical substance
    def __init__(self, substanceParams):   #sets up the atom / molecule
        # substanceParams = [[substanceType, atomNumber, x, y, xdir, ydir, substanceID, angleStartPos, rotationDirection, frameN, recentColl, frameWidth, productPair, velocityDisplayScale, centralAtom],
        ...
        self.willReact = 0 #once a reaction is detected, this is assigned to the substanceID of the substance(s) that this substance will react with
...
def substanceSetup(dimensions, atomInfo, frameWidth, substanceGenInfo,TStart,avgMr,velocityDisplayScale):
    ...
    substanceID = 1 #unique ID for each substance on screen
...
def reactionProcessing(substances, frameN, substanceID, atomInfo, substance1, substance2, reactionSuccessful, threeSubstanceReaction, velocityDisplayScale, substanceGenInfo):
    ...
    for substance3 in substances:
        ...
        if reactionSuccessful == 1:
            ...
            if productNumber == 1:
                substanceParams = [[substance1.product, 0, newx, newy, newVelocity[0], newVelocity[1], substanceID, angleStartPos, rotationDirection, frameN, [], substance1.frameWidth, 0, velocityDisplayScale, centralAtom[0]]] #new substance properties
                substanceParams = getSubstanceAtoms(substanceParams, atomInfo)
                substanceNew = substanceGen(substanceParams)
            ...
            if productNumber > 1:
               ...
               # v2 = np.array([substance2.velocity[0], substance2.velocity[1]])
                ...
                if productNumber == 3:
                    ...
                    triangleOffsets = [pg.math.Vector2(triangleRadius, 0), pg.math.Vector2(triangleRadius * math.cos(2 * math.pi / 3), triangleRadius * math.sin(2 * math.pi / 3)), pg.math.Vector2(triangleRadius * math.cos(4 * math.pi / 3), triangleRadius * math.sin(4 * math.pi / 3)),]
                ...
                substanceParams = [[products[0], 0, productPositions[0][0], productPositions[0][1], 0, 0, substanceID, angleStartPos, rotationDirection,\
                                    frameN, [substanceID+1], substance1.frameWidth, substanceID+1, velocityDisplayScale, centralAtom[0]]]
                substanceParams = getSubstanceAtoms(substanceParams, atomInfo)
...
def probabilityChecking(substances, reactionInfo, T):
    ...
    for r in reactionInfo:
        if str(r[0]) == reactants or str(r[1]) == reactants:
            ...
            EaF = r[3] #artificially very low (could also use a very high temp), needed for good reaction rate
            EaR = max(EaF - deltaG, 0)
            Ea = EaF if str(r[0]) == reactants else EaR
            p = np.exp(-Ea / (R * T))
            return random.random() < p  # Only return if match is found
    return False  # No valid reaction found

def reactionChecking(substances, frameN, substance1, substance2, toggleReaction,reactionInfo, T, reactionCount): #checks for reactions
            ...
            for i in range(1, substance1.iterations+2):
                ...
                if collDetected == 1 and toggleReaction == 1 and substance1.willReact == 0 and substance2.willReact == 0 and substance2.substanceID not in substance1.recentColl and substance1.substanceID not in substance2.recentColl:
                        ...
                        if nextReactionAllowed == 1:
                            reactionSuccessful2 = probabilityChecking([substance1, substance2], reactionInfo, T)
                                ...
                                if (collDetected == 1 or collDetected2 == 1) and substance3.willReact == 0:
                                    ...
                                    for r in reactionInfo:
                                        if r[0] == reactants or r[1] == reactants:
                                            ...
                                            break
                                    if nextReactionAllowed == 1:
                                        reactionSuccessful2 = probabilityChecking([substance1, substance2, substance3], reactionInfo, T)
                                        if reactionSuccessful2 == True:
...
def substanceUpdates(substances, frameN, velocityDisplayScale):
    for substance1 in substances: #need to update positions after all collisions from previous positions have been checked
        ...
        if substance1.atomNumber > 1:
            ...
            for i2 in range(0, substance1.atomNumber):
                ...
                pg.draw.circle(substance1.image, substance1.colour[i2], (qx, qy), substance1.atomRadii[i2])
                substance1.reactingAtomCentres[i2] = pg.math.Vector2(qx, qy) - substance1.com + substance1.reactingPos
        else:
        ...
        substance1.recentCollPrevious.append(copy.deepcopy(substance1.recentColl)) #if there are more than 10 frames' worth of info in substance1.recentCollPrevious
        if len(substance1.recentCollPrevious) > 10: #delete the oldest
            del substance1.recentCollPrevious[0]
...
def frameProcessing(substances, toggleReaction, frameN, substanceID, atomInfo,\
                 reactionInfo, avgMr, rollingData, framerate, substanceGenInfo, going, velocityDisplayScale, fig, ax1, ax2, scatter, scatter2, text_annotation, lineFit, showPlot, substanceOfInterest):  #processes each frame
    currentVels = [0]
    reactionCount = 0
        ...
        if substances.has(substance1): #checks whether substance1 hasn't reacted during the for loop 
            ...
            for substance2 in substances:
                if substances.has(substance2):
                    substances, substanceID = reactionProcessing(substances, frameN, substanceID, atomInfo, substance1, substance2,reactionSuccessful, threeSubstanceReaction, velocityDisplayScale, substanceGenInfo) #processes reactions from previous frame
    ...
    if frameN % 10 == 0 and showPlot:
        ...
        if len(validPoints) >= 2:
            ...
            text_annotation.set_text(f"calculated k = {kPerSecond:.4f} s⁻¹\nR² = {rSquared:.3f}")
...
def avgMrFromGenInfo(substanceGenInfo, atomInfo): #gets the average Mr from just substanceGenInfo rather than the list of generated substances (need to use this before substances have been generated)
    ...
    for substance in substanceGenInfo:
        ...
        if count == 0:
            continue
        # Prepare structure for getSubstanceAtoms
...
def main():
    ...
    if showPlot:
        ...
        text_annotation = ax1.text(0.3, 0.95, "", transform=ax1.transAxes, fontsize=10, verticalalignment='top')
...
##                    df = pd.DataFrame(outputList)
        ...
        substanceID, rollingData, going = frameProcessing(substances, toggleReaction, frameN, substanceID, atomInfo,\
                 reactionInfo, avgMr, rollingData, framerate, substanceGenInfo, going, velocityDisplayScale, fig, ax1, ax2, scatter, scatter2,
                                                          text_annotation, lineFit, showPlot, substanceOfInterest)
