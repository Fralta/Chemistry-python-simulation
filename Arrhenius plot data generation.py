...
def reactionProcessing(substance1, substance2,reactionSuccessful, threeSubstanceReaction, physicalParams, physicalParamsFields, simulationParams, simulationParamsFields):
    dimensions, atomInfo, frameWidth, substanceGenInfo, TStart, avgMr, velocityDisplayScale,\
                 iterations, substanceParamsMoleculeFields, boxes, grid = (physicalParams[i] for i in physicalParamsFields)
    substances, toggleReaction, frameN, substanceID, reactionInfo, rollingData, framerate, going, showPlot, timeInterval, EaMethod, trial\
...
def frameProcessing(physicalParams, physicalParamsFields, simulationParams, simulationParamsFields):  #processes each frame
    dimensions, atomInfo, frameWidth, substanceGenInfo, TStart, avgMr, velocityDisplayScale,\
                 iterations, substanceParamsMoleculeFields, boxes, grid = (physicalParams[i] for i in physicalParamsFields)
    substances, toggleReaction, frameN, substanceID, reactionInfo, rollingData, framerate, going, showPlot, timeInterval, EaMethod, trial\
                = (simulationParams[i] for i in simulationParamsFields)
    ...
    rate = reactionCount
    originalN2Count = next(substance["count"] for substance in substanceGenInfo if substance['formula'] == "N2")
    conversion = sum(s.substanceType == "NO" for s in substances) * 100 / (originalN2Count * 2) # * 2 to correct for stoichiometry
    ...
    if conversion >= 20: #once 20% conversion is reached
        ...
        print("Completed run "+str(trial+1)+" of 20")
...
def main(TStart, trial):
    ...
    simulationParamsFields = ["substances", "toggleReaction", "frameN", "substanceID", "reactionInfo", "rollingData",
                              "framerate", "going", "showPlot", "timeInterval", "EaMethod", "trial"]
    ...
    simulationParams = dict(zip(simulationParamsFields, [substances, toggleReaction, frameN, substanceID, reactionInfo,
                                            rollingData, framerate, going, showPlot, timeInterval, EaMethod, trial]))
    ...
    while simulationParams["going"]:
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN:
                if event.key == pg.K_ESCAPE:
                    simulationParams["going"] = False  #will quit when escape key is pressed
        ...
        frameProcessing(physicalParams, physicalParamsFields, simulationParams, simulationParamsFields) #processes current frame
