...
def frameProcessing(physicalParams, physicalParamsFields, simulationParams, simulationParamsFields, plotParams):  #processes each frame
    ...
    substances, toggleReaction, frameN, substanceID, reactionInfo, rollingData, framerate, going, showPlot, timeInterval, EaMethod\
                = (simulationParams[i] for i in simulationParamsFields)
    fig, ax1, ax2, scatter, scatter2, lineFit, textAnnotation = plotParams["fig"], plotParams["ax1"], plotParams["ax2"], plotParams["scatter"],\
                                                                plotParams["scatter2"], plotParams["lineFit"], plotParams["textAnnotation"]
    currentVels = [0]
    reactionCount = 0
    ...
    for reaction in reactionInfo.values():
        ...
        products = [s.strip() for s in reaction["products"].split('+') if s.strip()]  
        if reaction["k"] != "-":  #only consider specific first-order reaction with a specified k value
            reactantOfInterest = reactant
            actualk = str(reaction["k"])
...
def main():
    ...
    if showPlot:
        ...
        textAnnotation = ax1.text(0.1, 0.1, "", transform=ax1.transAxes, fontsize=10, verticalalignment='bottom', zorder = 4)
    ...
    plotParams = dict(zip(["fig", "ax1", "ax2", "scatter", "scatter2", "lineFit", "textAnnotation"],\
                           [fig, ax1, ax2, scatter, scatter2, lineFit, textAnnotation]))
